Set-Alias gcfg Get-PSFConfig
Set-Alias scfg Set-PSFConfig
Remove-PSFAlias -Name select -Force
Set-Alias select Select-PSFObject
Import-Module PSUtil
Add-Type -Path ("{0}\bin\dbatools.dll" -f (Get-Module dbatools -ListAvailable | Sort-Object Version -Descending | Select-Object -First 1).ModuleBase)

#region Expand Alias
Set-PSReadlineKeyHandler -Chord 'Alt+Q' -BriefDescription ExpandAlias -Description "Converts aliases into the resolved command / parameter" -ScriptBlock {
